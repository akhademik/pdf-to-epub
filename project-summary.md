...
2.  **Backend (`src/lib/server/index.ts`)**:
    ...
    - **`/api/convert` (POST)**:
      ...
      - Applies a series of text corrections using a predefined dictionary (`correction-dictionary.json`) to fix common OCR errors.
      ...
      - Identifies and logs "abnormal" words (words not found in a Vietnamese dictionary) to a separate file for later review.
